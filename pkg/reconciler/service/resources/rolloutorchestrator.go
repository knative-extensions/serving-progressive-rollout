...
func UpdateFinalTargetRev(ultimateRevisionTarget []v1.TargetRevision, ro *v1.RolloutOrchestrator) *v1.RolloutOrchestrator {
	if !trafficEqual(ro.Spec.TargetRevisions, ultimateRevisionTarget) {
		// If ultimateRevisionTarget is not equal to the TargetRevisions in the spec(The user updated the ksvc, leading to
		// the new rollout), it means the RolloutOrchestrator will start a new rollout, so we update
	...
	// If ultimateRevisionTarget is equal to the TargetRevisions in the spec(No update happened on the ksvc), it means
	// it is still in the progress of rolling out the new revision, so there is no change to the RolloutOrchestrator.
	return ro
}
