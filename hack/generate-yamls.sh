...
readonly SERVING_PR_CORE_YAML=${YAML_OUTPUT_DIR}/"serving-progressive-rollout-core.yaml"
readonly SERVING_PR_CRDS_YAML=${YAML_OUTPUT_DIR}/"serving-progressive-rollout-crds.yaml"
...
echo "Building Knative Serving Progressive Rollout"
# Create Knative Serving Progressive Rollout core yaml
ko resolve ${KO_YAML_FLAGS} -R -f config/core/ | "${LABEL_YAML_CMD[@]}" > "${SERVING_PR_CORE_YAML}"

# Create Knative Serving Progressive Rollout crds yaml
ko resolve ${KO_YAML_FLAGS} -f config/core/300-resources/ | "${LABEL_YAML_CMD[@]}" > "${SERVING_PR_CRDS_YAML}"

all_yamls=(${SERVING_PR_CORE_YAML} ${SERVING_PR_CRDS_YAML})
